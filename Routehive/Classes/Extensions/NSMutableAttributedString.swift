...
extension NSMutableAttributedString {
    ...
    func attributedString(withText text:String, color: UIColor = .black, textColor: UIColor = .black ,colorChnageString: [String]?, linkable : Bool = false, links : [String]?, font : UIFont) -> NSMutableAttributedString {
        
        let attrs:[NSAttributedStringKey: Any] = [NSAttributedStringKey.foregroundColor: textColor, .font : font]
        let attributedString = NSMutableAttributedString(string: text, attributes:attrs)
        
        if let colorChnageString = colorChnageString {
            
            for (index, colorChangeText) in colorChnageString.enumerated() {
                let range = (text as NSString).range(of: colorChangeText)
                
                if linkable {
                    attributedString.addAttribute(NSAttributedStringKey.link, value: links?[index] ?? "http://www.google.com", range: range)
                }
    ...
    @discardableResult func normal(_ text:String)->NSMutableAttributedString {
        let normal = NSAttributedString(string: text, attributes: [NSAttributedStringKey.foregroundColor : UIColor.black])
        self.append(normal)
        return self
    }
}
